...
/*
  InitList1(LinkList& L)：带头结点的链表初始化函数。该函数分配内存空间，并将头节点的指针指向该空间。
...
/*
  InitList(LinkList& L)：不带头结点的链表初始化函数。该函数将头节点的指针初始化为空，表示链表为空。
*/
...
/*
   ListInsert1(LinkList& L, int i, ElemType e)：带头结点的链表插入函数。该函数在链表的第 i 个位置
插入一个值为 e 的节点。首先检查 i 的合法性，然后遍历链表找到第 i - 1 个位置的节点 p。新建一个节点 s，将其 data 域赋
值为 e，将其 next 域指向 p 的后继节点 p->next，再将 p 的 next 域指向 s，完成插入操作。
*/
...
/*
   ListInsert(LinkList& L, int i, ElemType e)：不带头结点的链表插入函数。该函数在链表的第 i 个位置插入一个值
为 e 的节点。首先检查 i 的合法性，然后遍历链表找到第 i - 1 个位置的节点 p。如果要在第一个位置插入节点，新建一个
节点 s，将其 data 域赋值为 e，将其 next 域指向原来的第一个节点 L，再将头指针 L 指向 s，完成插入操作。如果不是
在第一个位置插入节点，新建一个节点 s，将其 data 域赋值为 e，将其 next 域指向 p 的后继节点 p->next，再将 p 的
next 域指向 s，完成插入操作。
*/
...
/*
   del_x1(LinkList& L, ElemType x)：带头结点的链表删除节点函数。该函数遍历链表，找到值为 x 的节点并删除。
如果当前节点的值为 x，删除该节点并递归调用函数，继续删除下一个节点；如果当前节点的值不为 x，继续向下遍历。
*/
...
/*
del_x(LinkList& L, ElemType x)：不带头结点的链表删除节点函数。该函数遍历链表，找到值为 x 的节点并删除。
如果当前节点的值为 x，删除该节点并递归调用函数，继续删除下一个节点；如果当前节点的值不为 x，继续遍历下一个
节点，继续判断是否为值为 x 的节点，直到链表末尾。
*/
